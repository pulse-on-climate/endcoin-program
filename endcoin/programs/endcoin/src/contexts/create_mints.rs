...
use crate::{
    constants::{AMM_SEED, AUTHORITY_SEED, META_LIST_ACCOUNT_SEED}, get_meta_list_size, get_mint_extensible_extension_data, get_mint_extension_data, update_account_lamports_to_minimum_balance 
};
...
impl<'info> CreateMints<'info> {
    pub fn initialize_token_metadata(
        ...
        match token {
            ...
            // Set mint to pool - Do we need to do this here? 
...
pub fn handler(ctx: Context<CreateMints>, token: bool) -> Result<()> {
    
    // initialize token metadata
    ctx.accounts.initialize_token_metadata(token)?;
